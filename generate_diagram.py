...
def draw_box(ax, x, y, w, h, text, color, fontsize=8.5, bold=False, alpha=0.85):
...
def draw_arrow(ax, x1, y1, x2, y2, label="", color=COL_EDGE, style="-|>",
               linestyle="-", connectionstyle="arc3,rad=0", linewidth=1.5):
...
def main():
    ...
    # Execution layer (right column) — aligned with execute_actions node
    draw_cluster(ax, 0.76, 0.02, 0.22, 0.28, "Execution Layer\n(Action Targets)", COL_EXECUTION)
    ...
    for sx, sy, stxt in sources:
        draw_box(ax, sx, sy, bw, bh, stxt, COL_PERCEPTION, fontsize=7.5)
    ...
    for i, (nx, ny, ntxt) in enumerate(nodes):
        ...
        draw_box(ax, nx, ny, nw, nh, ntxt, col, fontsize=7.5, bold=(i == 5))

    # ── Execution boxes (right) — aligned with execute_actions ──────
    targets = [
        (0.87, 0.17, "Slack / Email\nNotifications"),
        (0.87, 0.07, "ERP System\n(PO Updates)"),
    ]
    for tx, ty, ttxt in targets:
        draw_box(ax, tx, ty, bw, bh, ttxt, COL_EXECUTION, fontsize=7.5)

    # ── Human actor (right side) — aligned with human_review ──────
    draw_box(ax, 0.87, 0.26, bw, bh, "Procurement\nManager (Human)", COL_HUMAN, fontsize=7.5, bold=True)
    ...
    draw_arrow(ax, 0.54, 0.591, 0.54, 0.409, label="no alternatives",
    ...
    # ── Arrows: Reasoning → Execution (horizontal, same level) ─────
    # execute_actions → Slack/Email
    draw_arrow(ax, 0.60, 0.15, 0.78, 0.17, color=COL_EXECUTION,
               connectionstyle="arc3,rad=-0.05")
    # execute_actions → ERP
    draw_arrow(ax, 0.60, 0.13, 0.78, 0.07, color=COL_EXECUTION,
               connectionstyle="arc3,rad=0.05")
